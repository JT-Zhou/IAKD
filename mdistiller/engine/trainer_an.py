...
class ANTrainer(object):
    ...
    def train_epoch(self, epoch):
        ...
        if epoch <= 100:
            if epoch % int(self.cfg.SOLVER.EPOCHS / self.cfg.AN.T_MAX) == 0 and epoch > 0:
                self.temper += 1
        # train loops
        self.T = self.temper/self.cfg.AN.T_MAX

        self.distiller.train()
        for idx, data in enumerate(self.train_loader):
            msg = self.train_iter(data, epoch, train_meters,self.T)
    ...
    def train_iter(self, data, epoch, train_meters,T):
        ...
        # forward
        preds, losses_dict = self.distiller(image=image, target=target, epoch=epoch,T=T)
