...
class IATrainer(object):
    ...
    def train_epoch(self, epoch):
        ...
        if epoch in change_time:
            l_loss = list(self.ten[:,0])
            l_pt = list(self.ten[:,1])
            na_kd = np.array(l_loss).reshape(-1, 1)
            na_ce = np.array(l_pt).reshape(-1, 1)
            na_ce = min_max_scaler.fit_transform(na_ce)
            na_kd = min_max_scaler.fit_transform(na_kd)
            t = self.cfg.IAKD.A*na_kd + self.cfg.IAKD.B*na_ce
